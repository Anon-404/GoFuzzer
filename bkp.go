...
func printProgressBar(current, total int) {
	...
	// Save cursor position, print the progress bar, and restore cursor position
	fmt.Print("\0337") // Save cursor position
	fmt.Printf("\r[%s>%s] %d/%d", strings.Repeat("=", completed), strings.Repeat(" ", remaining), current, total)
...
func main() {
    ...
    var wg sync.WaitGroup // Ensure the WaitGroup is declared
    ...
    for _, word := range wd {
        ...
        go func(word string) {
            defer wg.Done() // Ensure Done is called at the end of the goroutine
        ...
        }(word) // Pass `word` as an argument to the anonymous function
        
        
        //printProgressBar(i, len(wd))
    }
    
    for tsk := 0; tsk <= len(wd); tsk++ {
        // Simulate work
        time.Sleep(50 * time.Millisecond)

        // Update progress bar
        printProgressBar(tsk, len(wd))
    }
